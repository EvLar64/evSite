#
# Python NLTK, NLP, Wordnet Assignment Part III (Additions for third part below) - EL
#
...
output_file = open(f"nlp_output_{timestamp}.txt", "w", encoding="utf-8")
...
# attempting some other synset functions I found,
# like hypernyms (general) and hyponyms (specific)
school = wn.synset('school.n.01')
print(school.hypernyms())
print(school.hyponyms())
...
plt.ylabel('Count')
plt.savefig(f"ferris_pos_{timestamp}.png")
...
plt.ylabel('Count')
plt.savefig(f"breakfast_pos_{timestamp}.png")
...
plt.ylabel('Count')
plt.savefig(f"sixteen_pos_{timestamp}.png")
plt.show()
